    ...
    public class NormalState : BaseActionState {
        ...
        public override EActionState Update(float deltaTime) {
            ...
            if (GameInput.GrabButton.Checked() && !player.Ducking) {
                //Climbing
                if (player.PlayerVelocity.y <= 0 && Math.Sign(player.PlayerVelocity.x) != -(int)player.Facing) {
            ...
            } else if (player.OnGround && player.MoveY == -1 && player.PlayerVelocity.y <= 0) {
                player.Ducking = true;
                Debug.Log("Ducking");
            ...
            if (player.Ducking && player.OnGround) {
                ...
                player.PlayerVelocity.x = Mathf.MoveTowards(player.PlayerVelocity.x, Constants.DuckSpeed * this.player.MoveX, Constants.DuckFriction * deltaTime);
            } 
            else {
                float mult = player.OnGround ? 1 : Constants.AirMult;
                float maxRun = player.Holding == null ? Constants.HoldingMaxRun : Constants.MaxRun;
                if (Math.Abs(player.PlayerVelocity.x) > maxRun && Math.Sign(player.PlayerVelocity.x) == this.player.MoveX) {
                    //同方向加速
                    player.PlayerVelocity.x = Mathf.MoveTowards(player.PlayerVelocity.x, maxRun * this.player.MoveX, Constants.RunReduce * mult * deltaTime);
                }
                else {
                      //不同方向加速
                    player.PlayerVelocity.x = Mathf.MoveTowards(player.PlayerVelocity.x, maxRun*this.player.MoveX, Constants.RunAccel*mult*deltaTime);
                }
            }
            #endregion
            #region 空中移动
            ...
            if (player.MoveY == -1 && player.PlayerVelocity.y <= maxFallSpeed) {
            ...
            if (!player.OnGround) { 
                ...
                if ((player.MoveX == (int)player.Facing || (player.MoveX == 0 && GameInput.GrabButton.Checked())) && player.MoveY != -1) {
                    //判断是否向下做Wall滑行
                    if (player.PlayerVelocity.y <= 0 && player.WallSlideTimer > 0 && player.ClimbBoundsCheck((int)player.Facing) && player.CollideCheck(player.Position, Vector2.right * (int)player.Facing) && player.CanUnDuck) {
                        player.Ducking = false;
                ...
                float mult = (Math.Abs(player.PlayerVelocity.y) < Constants.HalfGravThreshold && (GameInput.JumpButton.Checked())) ? .5f : 1f;
                player.PlayerVelocity.y = Mathf.MoveTowards(player.PlayerVelocity.y, max, Constants.Gravity * mult * deltaTime);
            }
            if (player.VarJumpTimer > 0) {
                if (GameInput.JumpButton.Checked()) {
                    //如果按住跳跃，则跳跃速度不受重力影响。
                    player.PlayerVelocity.y = Math.Max(player.PlayerVelocity.y, player.VarJumpSpeed);
