...
namespace Game {
    public class ClimbLadderState : BaseActionState {
        ...
        public override void OnBegin() {
            ...
            player.PlayerVelocity.y *= Constants.ClimbGrabYMult;
        ...
        public override EActionState Update(float deltaTime) {
            ...
            if (player.ClimbNoMoveTimer <= 0) {
                if (player.MoveY == 1) {
                    ...
                    if (player.CollideCheck(player.Position, Vector2.up)) {
                        ...
                        player.PlayerVelocity.y = Mathf.Min(player.PlayerVelocity.y, 0);
                        target = 0;
                        trySlip = true;
                    } 
                    ...
                    if (player.OnGround) {
                        player.PlayerVelocity.y = Mathf.Max(player.PlayerVelocity.y, 0);
                        target = 0;
                    } else {
                        // TODO: effect
                    }
            ...
            player.PlayerVelocity.y = Mathf.MoveTowards(player.PlayerVelocity.y, target, Constants.ClimbAccel * deltaTime);
            //TrySlip导致的下滑在碰到底部的时候,停止下滑
            if (player.MoveY != -1 && player.PlayerVelocity.y < 0 && !player.CollideCheck(player.Position, new Vector2((int)player.Facing, -1))) {
        ...
        private void ClimbHop() {
            ...
            //}
                ...
                player.HopWaitX = 0;
                player.PlayerVelocity.x = (int)player.Facing * Constants.ClimbHopX;
            }

            player.PlayerVelocity.y = Math.Max(player.PlayerVelocity.y, Constants.ClimbHopY);
            player.ForceMoveX = 0;
